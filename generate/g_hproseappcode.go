...
func GenerateHproseAppcode(driver, connStr, level, tables, currpath string) {
	...
	default:
		asanaLogger.Log.Fatal("Invalid 'level' option. Level must be either \"1\", \"2\" or \"3\"")
	}
	...
	default:
		asanaLogger.Log.Fatalf("Unknown database driver '%s'. Driver must be one of mysql, postgres or sqlite", driver)
	}
...
func writeHproseModelFiles(tables []*Table, mPath string, selectedTables map[string]bool) {
	...
	for _, tb := range tables {
		...
		if _, err := f.WriteString(fileStr); err != nil {
			asanaLogger.Log.Fatalf("Could not write model file to '%s'", fpath)
		}
		utils.CloseFile(f)
